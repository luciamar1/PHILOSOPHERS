...
void	calculate_thread_death(t_2link_circ_list *vars)
{
	...
	while (1)
	{
		...
		// printf("start: %lds %dms | end: %lds %dms\n", vars->start_eating.tv_sec, vars->start_eating.tv_usec, end.tv_sec, end.tv_usec);
		time_elapsed = ((end.tv_sec - vars->start_eating.tv_sec) * 1000) + ((end.tv_usec - vars->start_eating.tv_usec) / 1000);
		// printf("START == %ld \n", (end.tv_sec - vars->start_eating.tv_sec));
		// printf("1 == %ld      2 == %d\n", (vars->start_eating.tv_sec) , 1000 + (end.tv_usec - vars->start_eating.tv_usec) / 1000);
		// printf("CALCULANDO MUERTE.... tiempo de morir == %d      cuanto llrvaa = %lld\n", (vars->routine.time_to_die), time_elapsed);
